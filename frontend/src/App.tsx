...
export default function App() {
  ...
  // Synchronize dynamic sector analysis whenever an operational sector is active in the dashboard
  useEffect(() => {
    // A deep link like `?sector=joshimath-subsidence` is applied by the pin-hydration
    // effect below, which lands one commit later. Querying before that would analyse
    // the default sector, and its (slower) response would then fly the map away from
    // the sector the user actually asked for. Wait for hydration to settle.
    if (urlState.sector && selectedPin?.id !== urlState.sector) {
      return;
    }
    if (!isLandingMode && selectedPin && lastAnalyzedSectorRef.current !== `${selectedPin.id}-${activeModality}`) {
      const pinChanged = lastAnalyzedPinRef.current !== selectedPin.id;
      lastAnalyzedSectorRef.current = `${selectedPin.id}-${activeModality}`;
      lastAnalyzedPinRef.current = selectedPin.id;
      // A modality switch must re-run the query the user actually asked, otherwise
      // the deck (and now the agent's map actions) would answer the sector's default
      // prompt instead of the question on screen.
      const text = pinChanged || !queryResponse?.query ? selectedPin.recommendedQuery : queryResponse.query;
      executeQuery(text, selectedPin, activeModality, true);
    }
  }, [isLandingMode, selectedPin, activeModality, executeQuery, urlState.sector, queryResponse?.query]);
  ...
  const runLaunchFlight = useCallback(
    (targetPin: TacticalGlobePin) => {
      ...
      const map = (window as any).__MAP__;
      if (map) {
        // Continuous resize loop across both zoom-in (expanding to full screen) and glide-left (docking)
        let keepResizing = true;
        const resizeLoop = () => {
          if (!keepResizing) return;
        ...
        // Step 1: Camera slowly rotates and zooms tight into India across the ENTIRE full screen
        try {
          map.flyTo({
            center: [78.9629, 20.5937],
            zoom: 4.8,
          ...
          console.warn('Step 1 flyTo failed:', err);
        }

        // Step 2: Camera zooms back OUT to tactical overview (zoom: 3.5) and glides container to left
        setTimeout(() => {
          setLaunchPhase('glide-left');
          try {
            map.flyTo({
              center: [targetPin.lon, targetPin.lat],
              zoom: 3.5, // Explicitly zoomed out as requested by user to keep immersion!
              pitch: 30,
              bearing: 0,
              duration: 1100,
              curve: 1.25,
            ...
            console.warn('Step 2 flyTo failed:', err);
          }
        }, 1250);

        // Step 3: Complete transition into live dashboard with zoomed out perspective
        setTimeout(() => {
          keepResizing = false;
          setIsLaunching(false);
          setLaunchPhase('idle');
          ...
          navigate(`/dashboard?zoom=3.5&sector=${targetPin.id}&lat=${targetPin.lat}&lon=${targetPin.lon}`);
          if (map) {
            try {
              map.resize();
            } catch {}
  ...
  const handleSelectPin = useCallback(
    ...
    [activeModality, executeQuery, updateUrlState]
  );
    ...
    <div className="flex flex-col h-screen w-screen bg-[#080A0F] text-slate-100 overflow-hidden font-sans select-none relative">
      ...
      {isLandingMode && (
        <div
          ...
          className={`fixed inset-0 overflow-y-auto z-10 select-auto transition-opacity duration-500 ${isLaunching ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
          onScroll={(e) => {
        ...
        <div
          ...
          style={
            isLandingMode && launchPhase === 'idle'
              ...
              ? {
                  ...
                  left: 0,
                  top: 0,
                  width: typeof window !== 'undefined' && window.innerWidth >= 768 ? 'calc(100% - 384px)' : '100%',
                  height: '100%',
                  zIndex: 30,
          ...
          className={`border-none shadow-none rounded-none overflow-hidden ${
            isLandingMode && launchPhase === 'idle'
              ...
              : isLaunching
              ? 'fixed left-0 top-0 w-full h-full md:w-[calc(100%-384px)] z-30 opacity-100 pointer-events-auto'
