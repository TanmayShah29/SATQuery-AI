...
export interface QueryPromptBarProps {
  ...
  activeAoi?: [number, number, number, number] | null;
  onClearAoi?: () => void;
}
...
export const QueryPromptBar: React.FC<QueryPromptBarProps> = ({
  ...
  const [queryHistory, setQueryHistory] = useState<{ query: string; time: string; modality: string }[]>([
    {
      query: 'Detect building infrastructure expansions across Ahmedabad SAC',
      time: '10:14 UTC',
      modality: 'single_image',
    },
    {
      query: 'Pierce monsoon cloud cover with RISAT SAR in Brahmaputra flood plain',
      time: '09:48 UTC',
    ...
    if (queryResponse) {
      ...
      if (queryResponse.query) {
        setQueryHistory((prev) => {
          const exists = prev.some((h) => h.query === queryResponse.query);
      ...
      <div className="flex flex-col space-y-2">
        ...
        <div className="bg-[#0C1017]/95 border border-[#1E2638] hover:border-slate-500/60 rounded-xl p-2 sm:p-2.5 shadow-[0_12px_32px_rgba(0,0,0,0.85),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl transition-colors duration-150">
            ...
            <div className="flex items-center space-x-1.5 shrink-0 ml-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!showHistory) setShowSuggestions(false);
                }}
                title="View Session Query History & Follow-ups"
              ...
              <button
                ...
                onClick={() => {
                  ...
                  if (!showSuggestions) setShowHistory(false);
                }}
              ...
              {/* Opt-in live STAC scene ingestion */}
              <button
                type="button"
          ...
          <form onSubmit={handleSubmit} className="flex items-center space-x-2 mt-2">
            <div className="relative flex-1 flex items-center bg-[#070A10] border border-[#1E273A] rounded-lg px-2">
              {/* Active AOI Tag if Drawn */}
              {activeAoi && (
                <div className="flex items-center space-x-1 px-1.5 py-0.5 rounded bg-amber-500/20 border border-amber-500/40 text-amber-300 text-[10px] font-mono shrink-0 mr-1.5">
              ...
              <input
                ...
                placeholder={
                  activeAoi
                    ? `Query within custom drawn AOI...`
                    : lastSubmittedQuery
                    ? `Ask follow-up query... (e.g. 'How much area was modified?')`
                    : `Ask question or directive for ${activeSectorName}... (e.g. 'Identify structural deformation')`
                }
                disabled={isProcessing}
                className="w-full h-10 sm:h-11 bg-transparent text-xs text-slate-100 placeholder-slate-400 font-sans focus:outline-none"
              />
              {queryInput && (
                <button
                  type="button"
                  onClick={() => setQueryInput('')}
                  className="text-[9px] text-slate-400 hover:text-slate-200 font-mono cursor-pointer shrink-0 px-1"
                >
                  CLEAR
                </button>
              )}
          ...
          {queryResponse && !isProcessing && (
            <div className="mt-2 pt-1.5 border-t border-[#1C2433] flex items-center space-x-1.5 overflow-x-auto no-scrollbar py-0.5">
              <span className="text-[9px] font-mono text-slate-400 uppercase font-semibold shrink-0">
                FOLLOW-UP:
              </span>
              {[
                'Analyze surface area change magnitude',
                'Inspect through-cloud SAR radar backscatter (dB)',
                'Calculate NDVI & NDWI spectral indices',
                'Compare against baseline Cartosat high-res pass',
              ].map((chip, idx) => (
                <button
                  key={idx}
                  type="button"
                  ...
                  className="px-2 py-0.5 rounded-full bg-[#121824] hover:bg-[#1C2538] border border-[#222E42] hover:border-blue-500/40 text-[9px] font-mono text-slate-300 hover:text-white whitespace-nowrap transition-colors flex items-center space-x-1 shrink-0 cursor-pointer"
                >
                  <span>{chip}</span>
          ...
          {/* Collapsible Query History Drawer */}
          {showHistory && (
            <div className="mt-2 pt-1.5 border-t border-[#1C2433] space-y-1 max-h-40 overflow-y-auto font-mono text-xs">
              <div className="text-[9px] text-slate-400 uppercase font-semibold pb-1 flex items-center justify-between">
                <span>Recent Mission Queries</span>
                <button
                  type="button"
                  onClick={() => setQueryHistory([])}
                  className="hover:text-slate-200 text-[9px]"
                >
                  Clear History
                </button>
              </div>
              {queryHistory.map((h, i) => (
                <div
                  ...
                  onClick={() => {
                    ...
                    onExecuteQuery(h.query);
                    setShowHistory(false);
                  }}
                  className="p-1.5 rounded bg-[#101520] hover:bg-[#182030] border border-[#1E2638] flex items-center justify-between text-[10px] text-slate-300 hover:text-white cursor-pointer transition-colors"
                >
                  <span className="truncate pr-2">"{h.query}"</span>
                  <span className="text-slate-500 text-[9px] shrink-0">{h.time}</span>
                </div>
              ))}
            </div>
          )}
