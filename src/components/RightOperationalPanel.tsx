...
interface RightOperationalPanelProps {
  ...
  onExecutePrompt?: (prompt: string, mode?: ModalityMode) => void;
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-3.5 space-y-3.5">
          ...
          <div className="space-y-3">
            ...
            <div className="p-2.5 rounded-lg bg-gradient-to-b from-[#161C28] to-[#0E121B] border border-[#222A3B] space-y-2 text-xs font-mono shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-slate-500 text-[10px] block">DAG ORCHESTRATION PIPELINE</span>
                  <span className="text-slate-100 font-bold text-xs">
                    {queryResponse?.latency_ms != null ? `${queryResponse.latency_ms} ms Latency` : '5-Node Dynamic Graph'}
                  </span>
                </div>
                <div className="text-right">
                  <span className="text-slate-500 text-[10px] block">SYSTEM STATE</span>
                  <span className={`text-xs font-bold ${isAnalyzing ? 'text-cyan-400 animate-pulse' : queryResponse ? 'text-emerald-400' : 'text-amber-400'}`}>
                    {isAnalyzing ? 'ORCHESTRATING' : queryResponse ? 'RESOLVED' : 'ARMED & READY'}
                  </span>
                </div>
              </div>
              <div className="pt-1.5 border-t border-[#1F2738] flex items-center justify-between">
                <span className="text-slate-500 text-[10px] block">AUDIT PROVENANCE</span>
                <span className="text-slate-300 text-xs font-mono">
                  {auditHash || 'SHA-256 Verified On-Execution'}
                </span>
              </div>
            </div>
            ...
            {dagNodes.length === 0 ? (
              <div className="space-y-3">
                <div className="p-3 rounded-lg bg-[#0A0D15] border border-[#1B2335] space-y-2">
                  <div className="text-xs font-mono font-bold text-slate-200 flex items-center justify-between">
                    <span className="flex items-center gap-1.5">
                      <Network className="w-3.5 h-3.5 text-cyan-400" />
                      STANDBY PIPELINE TOPOLOGY
                    </span>
                    <span className="px-1.5 py-0.2 rounded text-[9px] bg-amber-950/60 text-amber-400 border border-amber-500/30">
                      IDLE
                    </span>
                  </div>
                  <p className="text-[11px] text-slate-400 font-sans leading-relaxed">
                    Executing any query initiates this 5-stage asynchronous Directed Acyclic Graph with sub-second execution telemetry and cryptographic proof:
                  </p>
                </div>

                {/* 5 Architecture Preview Nodes */}
                <div className="space-y-2 relative before:absolute before:left-3.5 before:top-4 before:bottom-4 before:w-0.5 before:bg-[#1E2638]">
                  {[
                    { step: '01', title: 'Spatial Retrieval & Extent Resolution', desc: 'Queries STAC catalog for Cloud-Optimized GeoTIFFs (Sentinel-2 / MOSDAC)', model: 'STAC Ingestion' },
                    { step: '02', title: 'Multi-Spectral Preprocessing', desc: 'Preprocesses RGB, NIR and SAR radar channels for target AOI', model: 'Spectral Engine' },
                    { step: '03', title: 'Vision-Language Reasoning', desc: 'Evaluates ground telemetry against user directive and benchmark ground-truth', model: 'Qwen2.5-VL / Gemini' },
                    { step: '04', title: 'Vector Segmentation & Bounding', desc: 'Extracts georeferenced polygon features and coordinate boundaries', model: 'Polygon Extractor' },
                    { step: '05', title: 'Actuation & Provenance Audit', desc: 'Renders overlays on globe and generates verifiable SHA-256 audit token', model: 'Audit Agent' }
                  ].map((node) => (
                    <div
                      key={node.step}
                      className="relative pl-8 bg-[#0C101A] border border-[#1A2234] rounded-lg p-2.5 space-y-1 shadow-sm opacity-85 hover:opacity-100 transition-opacity"
                    >
                      <div className="absolute left-2 top-3 w-3 h-3 rounded-full bg-[#162132] border border-slate-600 flex items-center justify-center text-[8px] font-mono text-slate-400">
                        {node.step}
                      </div>
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[9px] font-mono px-1 rounded bg-[#131924] text-slate-400">
                          {node.model}
                        </span>
                      </div>
                ...
                <button
                  ...
                  onClick={() => {
                    if (onExecutePrompt) {
                      onExecutePrompt(selectedPin?.recommendedQuery || 'Analyze high-resolution optical and SAR imagery at this location.', 'single_image');
                    } else {
                      onRunAnalysis();
                    }
                  }}
                  className="w-full py-2.5 px-3 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-mono text-xs font-bold transition-all shadow-md flex items-center justify-center space-x-2 cursor-pointer active:scale-[0.98]"
                >
                  <Play className="w-3.5 h-3.5 fill-current" />
                  <span>DISPATCH PIPELINE ON {selectedPin?.name?.split(' ')[0] || 'TARGET'}</span>
                </button>
              </div>
            ) : (
              <>
        ...
        {activeTab === 'evidence' && (
          <div className="space-y-3">
            {/* START STATE: MISSION READINESS & OPERATIONAL TARGET HUB */}
            {!queryResponse && !isAnalyzing && (
              <div className="space-y-3">
                {/* 1. Target Dossier Card */}
                <div className="p-3.5 rounded-xl bg-gradient-to-b from-[#111726] to-[#0A0D15] border border-[#202B3E] shadow-lg space-y-2.5">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[10px] font-mono font-bold tracking-wider text-emerald-400 uppercase">
                        TARGET LOCKED // STANDBY
                      </span>
                    </div>
                    <span className="px-2 py-0.5 rounded text-[9px] font-mono bg-[#162032] text-slate-300 border border-[#23314B]">
                      DEFCON {selectedPin?.defcon || 4}
                    </span>
                  </div>

                  <div>
                    <h3 className="text-sm font-bold text-slate-100 font-sans flex items-center gap-1.5">
                      <span>{selectedPin?.name || 'Tactical Sector Overview'}</span>
                    </h3>
                    <div className="flex items-center gap-2 mt-1 text-[11px] font-mono text-slate-400">
                      <span className="flex items-center gap-1 text-slate-300">
                        <MapPin className="w-3 h-3 text-cyan-400" />
                        {selectedPin ? `${selectedPin.lat.toFixed(4)}°N, ${selectedPin.lon.toFixed(4)}°E` : 'Global Sector'}
                      </span>
                      <span>•</span>
                      <span className="text-amber-400 font-medium">
                        {selectedPin?.state || 'MONITORING'}
                      </span>
                    </div>
                  </div>

                  <p className="text-xs text-slate-300 leading-relaxed font-sans bg-[#070A11] p-2.5 rounded-lg border border-[#182234]">
                    {selectedPin?.description || 'Active geospatial sector under continuous ISRO SAC SIH26167 multi-modal surveillance.'}
                  </p>

                  <div className="pt-2 border-t border-[#1C2538] flex items-center justify-between text-[10px] font-mono text-slate-400">
                    <span className="text-slate-500">Constellation Pass:</span>
                    <span className="text-cyan-300 font-semibold truncate max-w-[210px]">
                      {selectedPin?.activePasses || 'Sentinel-2A Optical & Sentinel-1 SAR'}
                    </span>
                  </div>
                </div>

                {/* 2. Three-Step Quick Start Guide */}
                <div className="p-3 rounded-xl bg-[#0B0F19] border border-[#1A2336] space-y-2">
                  <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 uppercase tracking-wider pb-1 border-b border-[#161E2E]">
                    <span className="flex items-center gap-1.5 text-slate-300 font-bold">
                      <Compass className="w-3 h-3 text-cyan-400" />
                      How to Operate SatQuery
                    </span>
                    <span className="text-slate-500">3-Step Pipeline</span>
                  </div>

                  <div className="space-y-1.5 text-xs font-sans">
                    <div className="flex items-start gap-2 p-2 rounded-lg bg-[#0E1422] border border-[#192336]">
                      <div className="w-4 h-4 rounded-full bg-cyan-950 border border-cyan-500/40 text-cyan-300 flex items-center justify-center font-mono text-[10px] font-bold shrink-0 mt-0.5">
                        1
                      </div>
                      <div>
                        <div className="font-semibold text-slate-200 text-xs">Confirm Target or Draw AOI</div>
                        <p className="text-[11px] text-slate-400 leading-tight mt-0.5">
                          Centered on <span className="text-slate-200 font-medium">{selectedPin?.name}</span>. Click pins on globe or click <span className="text-cyan-300 font-mono text-[10px]">Draw AOI</span> on the map.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2 p-2 rounded-lg bg-[#0E1422] border border-[#192336]">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <span className="font-semibold text-slate-200 text-xs">Select Sensor Band</span>
                          <span className="px-1.5 py-0.2 rounded text-[9px] font-mono bg-cyan-950/80 text-cyan-300 border border-cyan-500/30 font-bold">
                            Active: {currentBand}
                          </span>
                        </div>
                        <p className="text-[11px] text-slate-400 leading-tight mt-0.5">
                          {currentBand === 'RGB' && 'True-color 10m visible optical imagery for visual inspection.'}
                          {currentBand === 'NIR' && 'False-color infrared for vegetation stress, soil moisture & burn scars.'}
                          {currentBand === 'SAR' && 'Cloud-piercing radar for all-weather day/night ground penetration.'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2 p-2 rounded-lg bg-[#0E1422] border border-[#192336]">
                      ...
                      <div>
                        <div className="font-semibold text-slate-200 text-xs">Launch Mission Directive</div>
                        <p className="text-[11px] text-slate-400 leading-tight mt-0.5">
                          Click any verified directive below or ask a custom question in the prompt bar at bottom.
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                {/* 3. Verified Sector Directives (One-Click Launch) */}
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 uppercase tracking-wider px-0.5">
                    <span className="flex items-center gap-1.5 text-amber-400 font-bold">
                      ...
                      Verified Directives for {selectedPin?.name?.split(' ')[0] || 'Target'}
                    </span>
                    <span className="text-slate-500">1-Click Launch</span>
                  </div>

                  {/* Directive 1: Primary Sector Analysis */}
                  <div className="p-3 rounded-xl bg-gradient-to-r from-[#111726] to-[#0D121F] border border-[#212C42] hover:border-cyan-500/60 transition-all duration-200 space-y-2 group shadow-sm">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-mono bg-[#162032] text-slate-400">
                        Optical VQA
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 font-sans italic leading-snug">
                      &ldquo;{selectedPin?.recommendedQuery || 'Analyze high-resolution optical and SAR imagery at this location.'}&rdquo;
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="w-full py-2 px-3 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-mono text-xs font-bold transition-all shadow-md flex items-center justify-center space-x-1.5 cursor-pointer active:scale-[0.98]"
                    >
                  ...
                  {/* Directive 2: Bi-Temporal Change Detection */}
                  <div className="p-3 rounded-xl bg-[#0D121E] border border-[#1E283C] hover:border-amber-500/60 transition-all duration-200 space-y-2 group shadow-sm">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-1.5 text-xs font-mono font-bold text-slate-200">
                        <Split className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                        <span>BI-TEMPORAL CHANGE DETECTION</span>
                      </div>
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-mono bg-[#1C1A14] text-amber-300/90 border border-amber-500/30">
                        T1 vs T2
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 font-sans italic leading-snug">
                      &ldquo;Detect multi-temporal surface deformation and structural changes across 2023-2024 at {selectedPin?.name || 'this sector'}.&rdquo;
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                        const q = `Detect multi-temporal surface deformation and structural changes across 2023-2024 at ${selectedPin?.name || 'this sector'}.`;
                        if (onExecutePrompt) {
                          onExecutePrompt(q, 'bitemporal');
                        } else {
                          onRunAnalysis();
                        }
                      }}
                      className="w-full py-2 px-3 rounded-lg bg-[#182030] hover:bg-[#202C42] border border-[#283650] hover:border-amber-500/40 text-slate-100 font-mono text-xs font-bold transition-all flex items-center justify-center space-x-1.5 cursor-pointer active:scale-[0.98]"
                    >
                      <span>RUN BI-TEMPORAL SCAN</span>
                      <ArrowRight className="w-3.5 h-3.5 text-amber-400" />
                    </button>
                  </div>

                  {/* Directive 3: Cloud-Piercing SAR Radar */}
                  <div className="p-3 rounded-xl bg-[#0D121E] border border-[#1E283C] hover:border-cyan-500/60 transition-all duration-200 space-y-2 group shadow-sm">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-1.5 text-xs font-mono font-bold text-slate-200">
                        <Radar className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
                        <span>SAR CLOUD-PIERCING RADAR</span>
                      </div>
                      <span className="px-1.5 py-0.5 rounded text-[9px] font-mono bg-[#0F1B2B] text-cyan-300 border border-cyan-500/30">
                        Sentinel-1 C-Band
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 font-sans italic leading-snug">
                      &ldquo;Pierce monsoon cloud and haze cover with Sentinel-1 SAR to map ground structures at {selectedPin?.name || 'this sector'}.&rdquo;
                    </p>
                    <button
                      type="button"
                      onClick={() => {
                        const q = `Pierce monsoon cloud and haze cover with Sentinel-1 SAR to map ground structures at ${selectedPin?.name || 'this sector'}.`;
                        if (onExecutePrompt) {
                          onExecutePrompt(q, 'cross_modal');
                        } else {
                          onRunAnalysis();
                        }
                      }}
                      className="w-full py-2 px-3 rounded-lg bg-[#182030] hover:bg-[#202C42] border border-[#283650] hover:border-cyan-500/40 text-slate-100 font-mono text-xs font-bold transition-all flex items-center justify-center space-x-1.5 cursor-pointer active:scale-[0.98]"
                    >
                      <span>RUN RADAR SAR SCAN</span>
                      <ArrowRight className="w-3.5 h-3.5 text-cyan-400" />
                    </button>
                  </div>
                </div>

                {/* 4. Constellation Telemetry Overview */}
                <div className="p-2.5 rounded-lg bg-[#090C13] border border-[#182030] text-[10px] font-mono space-y-1.5">
                  <div className="text-slate-500 uppercase tracking-wider flex items-center justify-between pb-1 border-b border-[#141A26]">
                    <span>Pipeline Telemetry & Grounding</span>
                    <span className="text-emerald-400 font-bold">ONLINE</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-slate-400 pt-0.5">
                    <div>
                      <span className="text-slate-600 block">VLM REASONER</span>
                      <span className="text-slate-200 font-semibold">Qwen2.5-VL / Gemini</span>
                    </div>
                    <div>
                      <span className="text-slate-600 block">STAC INGESTION</span>
                      <span className="text-slate-200 font-semibold">MOSDAC & Copernicus</span>
                    </div>
                    <div>
                      <span className="text-slate-600 block">DAG ORCHESTRATOR</span>
                      <span className="text-slate-200 font-semibold">5-Node Graph</span>
                    </div>
                    <div>
                      <span className="text-slate-600 block">PROVENANCE</span>
                      <span className="text-slate-200 font-semibold">SHA-256 Verified</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {isAnalyzing && (
              <div className="p-6 rounded-xl bg-[#0B0F19] border border-cyan-500/40 shadow-[0_0_30px_rgba(6,182,212,0.15)] space-y-4 text-center">
                <div className="relative w-14 h-14 mx-auto">
                  <div className="absolute inset-0 rounded-full border-2 border-cyan-500/30 animate-ping"></div>
                  <div className="w-14 h-14 rounded-full bg-cyan-950/60 border-2 border-cyan-400 flex items-center justify-center text-cyan-300">
                ...
                <div>
                  <div className="text-sm font-mono font-bold text-slate-100">
                    VLM Agentic DAG In Flight
                  </div>
                  <p className="text-xs text-slate-400 font-sans mt-1">
                    Orchestrating multi-spectral reasoners across <span className="text-cyan-300 font-semibold">{selectedPin?.name || 'target sector'}</span>
                  </p>
                </div>

                <div className="space-y-1.5 p-3 rounded-lg bg-[#07090F] border border-[#1A2336] text-[11px] font-mono text-left">
                  <div className="flex items-center space-x-2 text-emerald-400">
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    <span>1. Ingest Spatial Extents & AOI</span>
                  </div>
                  <div className="flex items-center space-x-2 text-cyan-300 animate-pulse">
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    <span>2. Spectral Analysis ({currentBand} Pass)</span>
                  </div>
                  <div className="flex items-center space-x-2 text-slate-500">
                    <Clock className="w-3.5 h-3.5" />
                    <span>3. Vision-Language Grounding & Bounding</span>
                  </div>
                  <div className="flex items-center space-x-2 text-slate-500">
                    <Clock className="w-3.5 h-3.5" />
                    <span>4. Cryptographic Provenance Hash</span>
                  </div>
                </div>
              </div>
            )}

            {/* EVIDENCE & FINDINGS REPORT (Rendered when queryResponse is active) */}
            {queryResponse && !isAnalyzing && (
              <>
                {/* Reset to Target Overview Affordance */}
                <div className="flex items-center justify-between p-2 rounded-lg bg-[#0B0F19] border border-[#1A2336] text-xs font-mono">
                  <button
                    type="button"
                    onClick={onResetAnalysis}
                    className="flex items-center space-x-1.5 text-cyan-400 hover:text-cyan-300 transition-colors cursor-pointer group font-semibold"
                    title="Return to sector briefing & curated directives"
                  >
                    <RotateCcw className="w-3.5 h-3.5 group-hover:-rotate-90 transition-transform" />
                    <span>Back to Target Overview</span>
                  </button>
                  <span className="text-[10px] text-slate-500">
                    Latency: {queryResponse.latency_ms} ms
                  </span>
                </div>
